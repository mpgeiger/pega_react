...
import {alertActions, assignmentActions, errorActions} from "./index";
    ...
    return dispatch => {
        ...
        caseService.getCaseCreationPage(id).then(
            data => {
                ...
                dispatch(assignmentActions.assignmentReviewMode(id));
...
function createCaseV2(id, content = {}) {
    return dispatch => {
        dispatch(request(id));

        return caseService.createCaseV2(id, content).then(
            aCase => {
                dispatch(assignmentActions.addOpenAssignment(aCase.ID));
                dispatch(getCase(aCase.ID));
                dispatch(assignmentActions.getAssignment(aCase.nextAssignmentID));
